$(document).ready(function () {
   ...
   $('.header-menu-item-link-wrapper').click(function () {
      if ($(this).parents('.header-menu-item').hasClass('active')) {
         ...
         $(this).parents('.header-menu-item').find('.header-submenu-wrapper').slideUp(300);
         ...
         $(this).parents('.header-menu-item').find('.header-submenu-wrapper').slideToggle(300);
   ...
   $('.footer-menu-title').click(function () {
      if ($(window).width() <= '501') {
         if ($(this).hasClass('active')) {
            $(this).parents('.footer-menu-col').find('.footer-menu-links').slideUp(300);
            $(this).removeClass('active');
         } else {
            $(this).parents('.footer-menu-col').find('.footer-menu-links').slideToggle(300);
   ...
   $('.map-bottom-item-top').click(function () {
      if ($(this).hasClass('active')) {
         $(this).parents('.map-bottom-item').find('.map-bottom-item-bottom').slideUp(300);
         $(this).removeClass('active');
      } else {
         $(this).parents('.map-bottom-item').find('.map-bottom-item-bottom').slideToggle(300);
         $(this).addClass('active');
      }
   })
   ...
   $('.staking-item-details-top').click(function () {
      ...
      if ($(this).hasClass('active')) {
         $(this).parents('.staking-item-details').find('.staking-item-details-bottom').slideUp(300);
         $(this).removeClass('active');
      } else {
         $(this).parents('.staking-item').addClass('active');
         $(this).parents('.staking-item-details').find('.staking-item-details-bottom').slideToggle(300);
         $(this).addClass('active');
      }
   })
   ...
   $('.referral-asked-item-top').click(function () {
      if ($(this).hasClass('active')) {
         $(this).parents('.referral-asked-item').find('.referral-asked-item-bottom').slideUp(300);
         $(this).removeClass('active');
      } else {
         $(this).parents('.referral-asked-item').find('.referral-asked-item-bottom').slideToggle(300);
         $(this).addClass('active');
      }
   })
   ...
   setTimeout(function () {
      if ($('.landstype-slide').length) {
         $('.landstype-bottom-l p:first-child').text($('.swiper-slide.swiper-slide-active .landstype-slide').attr('data-text'));
         $('.landstype-bottom-l p:last-child').find('span.green').text($('.swiper-slide.swiper-slide-active .landstype-slide').attr('data-num-1'));
         $('.landstype-bottom-l p:last-child').find('span.full').text($('.swiper-slide.swiper-slide-active .landstype-slide').attr('data-num-2'));
      }
   }, 100);

   for (let i = 0; i < $('.swap-block-field-row-bottom').length; i++) {
      if ($('.swap-block-field-row-bottom').eq(i).hasClass('disabled')) {
         $('.swap-block-field-row-bottom').eq(i).find('input').attr('disabled', 'disabled');
      }
   }
	
   for (let i = 0; i < $('.referral-table-search').length; i++) {
      if ($('.referral-table-search').eq(i).hasClass('disabled')) {
         $('.referral-table-search').eq(i).find('input').attr('disabled', 'disabled');
      }
   }
   ...
   for (let i = 0; i < $('.staking-modal-field').length; i++) {
      if ($('.staking-modal-field').eq(i).hasClass('disabled')) {
         $('.staking-modal-field').eq(i).find('input').attr('disabled', 'disabled');
      }
   }
   ...
   $('.swap-modal-item').click(function () {
      if (swapRow.find('.swap-block-field-row-top-text-icon img').length) {
         swapRow.find('.swap-block-field-row-top-text-icon img').attr('src', $(this).find('.swap-modal-item-l img').attr('src'));
      } else {
         swapRow.find('.swap-block-field-row-top-text-icon').append('<img>');
         swapRow.find('.swap-block-field-row-top-text-icon img').attr('src', $(this).find('.swap-modal-item-l img').attr('src'));
      }
      swapRow.find('.swap-block-field-row-top-text span:last-child').text($(this).find('.swap-modal-item-l span').text());
      $('.swap-modal-wrapper').removeClass('active');
      swapRow = '';
   })
